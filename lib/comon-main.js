...
function main() {
    ...
    if (getChapter()) {
        ...
        Array.from(imgElements).slice(0, (getCookie('scroll')||1)).forEach((c) => {
        ...
        window.onscroll = function() {
            imgElements.forEach((c) => {
                if (c.getAttribute('data-src') && (c.offsetTop < (window.scrollY+window.innerHeight))) {
                    setCookie('scroll', Array.from(imgElements).indexOf(c));
